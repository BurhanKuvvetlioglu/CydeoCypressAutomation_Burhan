...
describe('Context: My First Tests', () => {
    ...
    it('Check Different Button Actions', () => {
        ...
        // I will get all buttons like previous approach, get only the item, then check for each item
        // If it's equal to Button 4, then click on it
        cy.get('button').each((item) => {
            if (item.text() =="Button 4") {
                cy.log(item.next()); // this command writes the test at the test console              
                // item.click();   -- You cannot use cypress click function on jQuerry elements
                cy.wrap(item).click();
